...
import { Briefcase, ArrowLeft, Save, Sparkles, Building, AlertCircle, RefreshCw } from 'lucide-react';
...
export default function EditApplication() {
    ...
    async function loadApplication() {
      ...
      try {
        ...
        if (data.applied_date) {
          formattedDate = new Date(data.applied_date).toISOString().split('T')[0];
        }
        ...
        toast.error('The application record could not be loaded.', 'Application Not Found');
        navigate('/');
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!id) return;
    if (!validate()) {
      toast.error('Please resolve validation errors on form inputs.', 'Validation Error');
      return;
    }
    ...
    try {
      ...
      toast.success(`Application for ${form.company_name} updated successfully.`, 'Changes Saved');
      ...
      if (err.response?.data?.details) {
        ...
        toast.error('Could not save modifications. Try again.', 'Error');
      }
    } finally {
      setSubmitting(false);
    }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-3" id="edit-loading-stage">
        <RefreshCw className="w-8 h-8 text-blue-600 dark:text-blue-400 animate-spin" />
        <span className="text-sm text-slate-500 dark:text-slate-400 font-medium">Retrieving application history...</span>
      </div>
    );
  }

  return (
    <div className="max-w-2xl mx-auto space-y-6" id="edit-application-container">
      {/* Back link bar */}
      <button
        onClick={() => navigate('/')}
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-850 rounded-2xl p-6 sm:p-8 shadow-xs transition-colors duration-200">
        <div className="flex items-center gap-3 border-b border-slate-50 dark:border-slate-800 pb-5 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-950 dark:text-white font-sans">Edit Job Application</h1>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Edit application status timelines, role, and progress milestones.</p>
          </div>
        </div>

        <form onSubmit={handleSubmit} className="space-y-5" id="edit-application-form">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-50 dark:border-slate-800 mt-8">
            ...
            <button
              ...
              id="btn-edit-application-submit"
            >
              <Save className="w-4 h-4" />
              <span>{submitting ? 'Applying changes...' : 'Save Job Record'}</span>
            </button>
          </div>
        </form>
      </div>
